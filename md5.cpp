...
int build_reply_digest( const char* username, const char* realm, const char* secret,const char* method, const char* uri, const char* nonce,char* digest, int digest_len)
{
    ...
    char resp[256];
    char resp_hash[256];
    //mylog(5,"build_reply_digest");
    //struct sip_auth *auth = NULL;	/* Realm authentication */
    /* Calculate SIP digest response */
    snprintf(a1, sizeof(a1), "%s:%s:%s", username, realm, secret);    //_snprintf(a1, sizeof(a1), "%s:%s:%s", "771", "asterisk", "771");
    snprintf(a2, sizeof(a2), "%s:%s", method, uri);    //_snprintf(a2, sizeof(a2), "%s:%s", "REGISTER", "sip:192.168.0.252");
    /*if (!ast_strlen_zero(md5secret))        ast_copy_string(a1_hash, md5secret, sizeof(a1_hash));    else*/
    ...
    //mylog(5,"resp=%s",resp);    mylog(5,"resp_hash=%s",resp_hash);
    sprintf(digest, "Authorization: Digest username=\"%s\",realm=\"%s\",nonce=\"%s\",uri=\"%s\",response=\"%s\",algorithm=MD5",username, realm, nonce, uri, resp_hash);
